...
class ctNHP_Options extends NHP_Options {
	...
	function _options_page_html() {
		...
		submit_button( __( 'Reset to Defaults', 'nhp-opts' ),
			'secondary',
		...
		if ( isset( $_GET['settings-updated'] ) && $_GET['settings-updated'] == 'true' && get_transient( 'nhp-opts-saved' ) == '1' ) {
			if ( isset( $this->options['imported'] ) && $this->options['imported'] == 1 ) {
				echo '<div id="nhp-opts-imported">' . apply_filters( 'nhp-opts-imported-text-' . $this->args['opt_name'],
						__( '<strong>Settings Imported!</strong>', 'nhp-opts' ) ) . '</div>';
			} else {
				echo '<div id="nhp-opts-save">' . apply_filters( 'nhp-opts-saved-text-' . $this->args['opt_name'],
						__( '<strong>Settings Saved!</strong>', 'nhp-opts' ) ) . '</div>';
			}
			delete_transient( 'nhp-opts-saved' );
		}
		echo '<div id="nhp-opts-field-errors">' . __( '<strong><span></span> error(s) were found!</strong>',
				'nhp-opts' ) . '</div>';

		echo '<div id="nhp-opts-field-warnings">' . __( '<strong><span></span> warning(s) were found!</strong>',
				'nhp-opts' ) . '</div>';
		...
		foreach ( $groups as $key => $main ) {
			...
			if ( true === $this->args['show_import_export'] && $main == 'General' ) {
				...
				echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="import_export_default"><img src="' . esc_url($this->url . 'img/glyphicons/glyphicons_082_roundabout.png"').'" /> <span>' . __( 'Import / Export',
						'nhp-opts' ) . '</span></a>';
				echo '</li>';
				echo '<li class="divide">&nbsp;</li>';
			}
			...
			if ( true === $this->args['dev_mode'] && $main == 'General' ) {
				...
				echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="nhp-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default"><img src="' . esc_url($this->url . 'img/glyphicons/glyphicons_195_circle_info.png"').' /> <span>' . __( 'Dev Mode Info',
						'nhp-opts' ) . '</span></a>';
				echo '</li>';
			}
			//if
			...
			if ( true === $this->args['show_import_export'] && $main == 'General' ) {
				...
				echo '<p><a href="javascript:void(0);" id="nhp-opts-import-code-button" class="button-secondary">'.__('Import from file','nhp-opts').'</a> <a href="javascript:void(0);" id="nhp-opts-import-link-button" class="button-secondary">Import from URL</a></p>';
				...
				echo '<p class="description" id="import-code-description">' . apply_filters( 'nhp-opts-import-file-description',
						__( 'Input your backup file below and hit Import to restore your sites options from a backup.',
							'nhp-opts' ) ) . '</p>';
				...
				echo '<p class="description" id="import-link-description">' . apply_filters( 'nhp-opts-import-link-description',
						__( 'Input the URL to another sites options set and hit Import to load the options from that site.',
							'nhp-opts' ) ) . '</p>';
				...
				echo '<p id="nhp-opts-import-action"><input type="submit" id="nhp-opts-import" name="' . esc_attr($this->args['opt_name']) . '[import]" class="button-primary" value="' . __( 'Import',
						'nhp-opts' ) . '"> <span>' . apply_filters( 'nhp-opts-import-warning',
						__( 'WARNING! This will overwrite any existing options, please proceed with caution!',
							'nhp-opts' ) ) . '</span></p>';
				...
				echo '<p class="description">' . apply_filters( 'nhp-opts-backup-description',
						__( 'Here you can copy/download your themes current option settings. Keep this safe as you can use it as a backup should anything go wrong. Or you can use it to restore your settings on this site (or any other site). You also have the handy option to copy the link to yours sites settings. Which you can then use to duplicate on another site',
							'nhp-opts' ) ) . '</p>';
				echo '</div>';
