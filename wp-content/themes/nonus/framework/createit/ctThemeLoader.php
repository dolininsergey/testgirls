...
class ctThemeLoader {
	...
	public function setDocumentationUrl( $url, $label = '' ) {
		if ( ! $label ) {
			$label = __( "Documentation", 'ct_theme' );
		}
	...
	public function showLongDirNameError() {
       ...
       <p><strong>' . sprintf( __( "Your theme's directory name %s is too long! It will cause unpredictable errors in your Wordpres installation. Please change it's directory name to maximum 20 characters.",
					'ct_theme' ),
					$name ) . '</strong></p>
    </div>';
		}
