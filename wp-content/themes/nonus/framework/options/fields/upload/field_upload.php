...
class NHP_Options_upload extends NHP_Options {
	...
	function render() {
		...
		if (isset($this->field['show_dimensions']) && $this->field['show_dimensions']) {
			...
			echo '<br/><label' . $style . ' class="dimensions nhp-uploaded">' . __("Width:", 'ct_theme') . ' <input type="text" id="' . $this->field['id'] . '_width" name="' . $this->args['opt_name'] . '[' . $this->field['id'] . '_width]" value="' . esc_attr(ct_get_option($this->field['id'] . '_width')) . '" class="short ' . $class . '" /></label>';
			echo ' <label' . $style . ' class="dimensions nhp-uploaded">' . __("Height:", 'ct_theme') . ' <input type="text" id="' . $this->field['id'] . '_height" name="' . $this->args['opt_name'] . '[' . $this->field['id'] . '_height]" value="' . esc_attr(ct_get_option($this->field['id'] . '_height')) . '" class="short ' . $class . '" /></label>';
		} else {
		...
		echo ' <a href="javascript:void(0);" class="nhp-opts-upload-remove nhp-uploaded"' . $remove . ' rel-id="' . esc_attr($this->field['id']) . '">' . __('Remove Upload', 'nhp-opts') . '</a>';
